...
function extractTitleFromHref(href) {
    ...
    try {
        ...
        if (u.pathname.startsWith("/wiki/")) {
            const raw = decodeURIComponent(u.pathname.replace("/wiki/", "").split("#")[0]);
            return raw.replace(/_/g, " ");
        }
...
export default function WikiView({ startTitle, locked, }) {
    ...
    async function load(t) {
        ...
        try {
            ...
            const headings = Array.from(wrapper.querySelectorAll("h2, h3, section > header h2, section > header h3"));
            const shouldRemove = (text) => {
                const s = text.trim().toLowerCase();
                return (s.includes("liens externes") ||
            ...
            requestAnimationFrame(() => {
                ...
                if (c)
                    c.scrollTo({ top: 0, left: 0, behavior: "auto" });
            });
        }
        catch {
            setTitle(title);
    ...
    return (_jsxs("div", { className: "rounded border border-neutral-300 overflow-hidden bg-white", children: [_jsx("header", { className: "px-6 py-4 border-b border-neutral-300 bg-[#f8f9fa]", children: _jsx("div", { className: "text-2xl md:text-3xl font-serif font-semibold text-black mw-page-title-main", children: decodeURIComponent(title.replace(/_/g, " ")) }) }), _jsx("div", { ref: containerRef, className: "wiki-content max-w-none px-6 py-6 [&_img]:h-auto [&_img]:max-w-full [&_figure]:my-4 [&_table]:w-full [&_table]:border-collapse [&_th]:border [&_td]:border [&_th]:border-neutral-200 [&_td]:border-neutral-200 [&_th]:px-2 [&_td]:px-2 [&_th]:py-1 [&_td]:py-1", dangerouslySetInnerHTML: { __html: html } })] }));
}
